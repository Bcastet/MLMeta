...
class MatchAPI(RiotAPIService):
    ...
    _validate_get_match_list_query = Query. \
        ...
        has("startTime").as_(int).also. \
        can_have("endTime").as_(int).also. \
        has("start").as_(int).also. \
    ...
    def get_match_list(self, query: MutableMapping[str, Any], context: PipelineContext = None) -> MatchListDto:
        ...
        begin_time = query["startTime"]  # type: arrow.Arrow
        ...
        if calling_method == "by_date":
            params["startTime"] = begin_time.int_timestamp
            if "endTime" in query:
                params["endTime"] = min((end_time + riot_date_interval).int_timestamp, query["endTime"])
                params["start"] = query["start"]
                params["count"] = query["count"]
            else:
                params["endTime"] = (end_time + riot_date_interval).int_timestamp
                params["start"] = query["start"]
                params["count"] = query["count"]
        else:
            params["start"] = query["start"]
            params["count"] = query["start"] + min(riot_index_interval, query["maxNumberOfMatches"])
        ...
        if calling_method == "by_index":
            ...
            data["maxNumberOfMatches"] = params["maxNumberOfMatches"]
        else:
