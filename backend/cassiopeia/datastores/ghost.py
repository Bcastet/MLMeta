...
class UnloadedGhostStore(DataSource):
    ...
    _validate_get_match_history_query = Query. \
        ...
        can_have("start").as_(int).also. \
        can_have("count").as_(int).also. \
    ...
    def get_match_history(self, query: MutableMapping[str, Any], context: PipelineContext = None) -> MatchHistory:
        ...
        count = query.get("count", None)
        first_requested_dt = query.get("startTime", 0)  # (begin_time) Defaults to start of summoner's match history
        ...
        max_number_of_requested_matches = float("inf") if count is None else count - start

        # Convert times to datetimes
        first_requested_dt = arrow.get(first_requested_dt)
        if most_recent_requested_dt is not None:
            most_recent_requested_dt = arrow.get(most_recent_requested_dt)
            assert most_recent_requested_dt > first_requested_dt

        # Create the generator that will populate the match history object.
        def generate_matchlists(start: int, max_number_of_requested_matches: int = None, first_requested_dt: arrow.Arrow = None, most_recent_requested_dt: Union[arrow.Arrow, None] = None):
            _start = start
            ...
            while pulled_matches < max_number_of_requested_matches:
                new_query = {
                    ...
                    "puuid": puuid,
                    "start": _start,
                    "startTime": _current_first_requested_dt.int_timestamp,
                ...
                if hasattr(data, "count"):
                    expected_pulled_matches_this_iteration = count
                else:
                ...
                if expected_pulled_matches_this_iteration is not None and matches_pulled_this_iteration < expected_pulled_matches_this_iteration:
                    ...
                    break
                if most_recent_requested_dt is not None and matchrefdata is not None and Match(id = matchrefdata.id, continent=matchrefdata.continent).creation >= most_recent_requested_dt:
                ...
                if hasattr(data, "count"):
                    _start = data.count
                # else: Don't update _start
                if hasattr(data, "startTime"):
                    _current_first_requested_dt = arrow.get(data.startTime)
        ...
        generator = generate_matchlists(start, max_number_of_requested_matches, first_requested_dt, most_recent_requested_dt)

        generator = MatchHistory.from_generator(generator=generator,
                                                puuid=puuid, start=start, count=100,
                                                start_time=first_requested_dt, end_time=most_recent_requested_dt,
