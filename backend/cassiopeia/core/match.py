...
class ParticipantData(CoreData):
    ...
    def __call__(self, **kwargs):
        ...
        if self.perks == {} and kwargs!={}:
            for p in range(6):
                perk = str(p)
                self.perks[kwargs["stats"].pop("perk"+perk)] = [kwargs["stats"].pop("perk"+perk+"Var1"),kwargs["stats"].pop("perk"+perk+"Var2"),kwargs["stats"].pop("perk"+perk+"Var3")]
        if stat_perks == {} and kwargs!={}:
            types = ["defense","flex","offense"]
            for p in [0,1,2]:
                perk = str(p)
                stat_perks[types[p]] = kwargs["stats"].get("statPerk" + perk)
...
class MatchData(CoreData):
    ...
    def __call__(self, **kwargs):
        ...
        if "participantIdentities" in kwargs:
            participantNames = [p["player"]["summonerName"] for p in kwargs["participantIdentities"]]
        ...
        for index, participant in enumerate(participants):
            if "participantIdentities" in kwargs:
                participant["summonerName"] = participantNames[index]
...
class MatchHistory(CassiopeiaLazyList):  # type: List[Match]
    ...
    def __init__(self, *, puuid: str, continent: Continent = None, region: Region = None, platform: Platform = None, start: int = None, count: int = None, start_time: arrow.Arrow = None, end_time: arrow.Arrow = None, queue: Queue = None, type: MatchType = None):
        assert count is None or count > start
        if start_time is not None and end_time is not None and start_time > end_time:
            raise ValueError("`end_time` should be greater than `start_time`")
        kwargs = {"continent": continent, "puuid": puuid, "queue": queue, "type": type, "start": start, "count": count}
    ...
    @classmethod
    def __get_query_from_kwargs__(cls, *, continent: Continent, puuid: str, region: Region = None, platform: Platform = None, start: int = None, count: int = None, start_time: arrow.Arrow = None, end_time: arrow.Arrow = None, queue: Queue = None, type: MatchType = None):
...
class Participant(CassiopeiaObject):
    ...
    def team_position(self) -> Lane:
        try:
            ...
            competitive_lanes = ["TOP", "JUNGLE", "MID", "BOTTOM", "UTILITY"]
            return Lane.from_match_naming_scheme(competitive_lanes[(self.id -1)%5])
    ...
    def stats(self) -> ParticipantStats:
        if isinstance(self._data[ParticipantData].stats, dict):
            self._data[ParticipantData].stats = ParticipantStatsData(**self._data[ParticipantData].stats)
    ...
    def runes(self) -> Dict[Rune, int]:
        version = _choose_staticdata_version(self.__match)
        assert len(self._data[ParticipantData].perks.items())>0
    ...
    def summoner_spell_d(self) -> SummonerSpell:
        ...
        try:
            ...
            return SummonerSpell(id=self._data[ParticipantData].spell1Id, version=version,
    ...
    def summoner_spell_f(self) -> SummonerSpell:
        version = _choose_staticdata_version(self.__match)
        try:
            return SummonerSpell(id=self._data[ParticipantData].summonerSpellFId, version=version,
                                 region=self.__match.region)
        except:
            return SummonerSpell(id=self._data[ParticipantData].spell2Id, version=version,
                                 region=self.__match.region)
...
class Team(CassiopeiaObject):
    ...
    @property
    def dragon_kills(self) -> int:
        print(self._data[TeamData].objectives)
...
class Match(CassiopeiaGhost):
    ...
    def timeline(self) -> Timeline:
        if self._timeline is None:
            gameId = self.platform.value + "_" + str(self.id)
            self._timeline = Timeline(id=gameId, continent=self.continent)
